...
extern void install_allegro();
extern void allegro_init();
extern void allegro_init_all(int id, int w, int h, int menu, int enable_keys);
#define END_OF_MAIN()
...
extern void remove_int(procedure);
extern void remove_all_ints();
...
extern int remove_keyboard();

/* BITMAP ROUTINES */
typedef struct _bitmap_object {
	int w, h;
	void canvas, context;
	int ready;
	char *type;
...
extern int set_gfx_mode(char *canvas_id, int width, int height);
...
extern void makecol(char r, char g, char b, char a);
extern void makecolf(float r, float g, float b, float a);
...
extern void arc(BITMAP_OBJECT *bitmap, int x, int y, float ang1, float ang2, int r, int colour, int width);
extern void arcfill(BITMAP_OBJECT *bitmap, int x, int y, float ang1, float ang2, int r, int colour);

/* BLITTING AND SPRITES */
extern void draw_sprite(BITMAP_OBJECT *bmp,sprite, int x, int y);
extern void stretch_sprite(BITMAP_OBJECT *bmp,sprite, int x, int y, int w, int h);
extern void rotate_sprite(BITMAP_OBJECT *bmp,sprite, int x, int y, float angle);
extern void pivot_sprite(BITMAP_OBJECT *bmp,sprite, int x, int y, int cx, int cy, float angle);
extern void rotate_scaled_sprite(BITMAP_OBJECT *bmp,sprite, int x, int y, float angle, float scale);
extern void pivot_scaled_sprite(BITMAP_OBJECT *bmp,sprite, int x, int y, int cx, int cy, float angle, float scale);
...
/* TEXT OUTPUT */
typedef struct _font_object {
	void element;
	char *file, *name, *type;
} FONT_OBJECT:
extern FONT_OBJECT load_font(char *filename);
extern FONT_OBJECT create_font(char *family);
extern void textout(BITMAP_OBJECT *bitmap,FONT_OBJECT *f, char *s, int x, int y, int size, int colour, int outline, int width);
extern void textout_centre(BITMAP_OBJECT *bitmap, FONT_OBJECT *f, char *s, int x, int y, int size, int colour, int outline, int width);
extern void textout_right(BITMAP_OBJECT *bitmap, FONT_OBJECT *f, char *s, int x, int y, int size, int colour, int outline, int width);

/* SOUND ROUTINES */
typedef struct _sample_object {
	void element;
	char *file;
	float volume;
	int ready;
...
extern void enable_debug(char *id);
extern void log(char *string);
extern void wipe_log();
